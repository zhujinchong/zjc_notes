...
1. 有向图（贝叶斯网络）
2. 无向图（马尔可夫随机场）两个方面，例如 GMM
3. 动态模型：在这些基本的模型上，如果样本之间存在关联，可以认为样本中附带了时序信息，从而样本之间不独立全同分布的，这种模型就叫做动态模型，隐变量随着时间发生变化，于是观测变量也发生变化。
...
1. 问题是时序的
2. 问题中有两类数据：观测序列、状态序列

## HMM

HMM用参数 $\lambda=(\pi,A,B)$ 来表示，其中 $\pi$ 是开始的概率分布，$A$ 为状态转移矩阵，$B$ 为发射矩阵。
...
1.  Evaluation：$p(O|\lambda)$，Forward-Backward 算法。已知模型参数、观测序列，求观测序列出现的概率
2.  Learning：$\lambda=\mathop{argmax}\limits_{\lambda}p(O|\lambda)$，EM 算法（Baum-Welch）。已知多个观测序列，求模型参数
3.  Decoding：$I=\mathop{argmax}\limits_{I}p(I|O,\lambda)$，Vierbi 算法。已知模型参数、观测序列，求最有可能的状态序列
...
=\sum_{i_1} \sum_{i_2} \cdots \sum_{i_T} \pi_{i_1} \prod\limits_{t=2}^Ta_{i_{t-1},i_t}\prod\limits_{t=1}^Tb_{i_t}(o_t)
$$
我们看到，上面的式子中的求和符号是对所有的观测变量求和，于是复杂度为 $O(N^T)$。

下面，记t时刻前向概率 $\alpha_t(i)=p(o_1,o_2,\cdots,o_t,i_t=q_i|\lambda)$，所以，$\alpha_T(i)=p(O,i_T=q_i|\lambda)$。我们看到：
$$
...
=\mathop{argmax}_\lambda\sum\limits_I\log p(O,I|\lambda)\frac{p(O,I|\lambda^t)}{p(O|\lambda^t)} \\
=\mathop{argmax}_\lambda\sum\limits_I\log p(O,I|\lambda)p(O,I|\lambda^t)
$$
 上式利用了 $p(O|\lambda^t)$ 和$\lambda$ 无关。将 Evaluation 中的$p(O, I|\lambda)$ 式子代入：
$$
...
3.  fittering滤波：$ p(z_t|x_1,\cdots,x_t)$，Online，在线学习的方法
    $$
    p(z_t|x_{1:t})=\frac{p(x_{1:t},z_t)}{p(x_{1:t})} \propto C\alpha_t(z_t)
    $$

4.  smothing平滑：$p(z_t|x_1,\cdots,x_T)$，Offline
    $$
    p(z_t|x_{1:T})=\frac{p(x_{1:T},z_t)}{p(x_{1:T})}=\frac{\alpha_t(z_t)p(x_{t+1:T}|x_{1:t},z_t)}{p(x_{1:T})}
    $$
    根据概率图的条件独立性（$z_t$分割开了$x_{1: t}$和$x_{t+1:T}$），有：
    $$
    p(z_t|x_{1:T})=\frac{\alpha_t(z_t)p(x_{t+1:T}|z_t)}{p(x_{1:T})} \propto C\alpha_t(z_t)\beta_t(z_t)
    $$
    这个算法叫做前向后向算法。
